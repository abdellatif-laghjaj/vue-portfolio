<template>
  <div class="hero min-h-screen bg-base-200">
    <div class="hero-content text-center">
      <div class="max-w-lg mb-[80px] sm:mt-0">
        <!-- Avatar -->
        ...
        <!-- Name -->
        <h3
          class="text-3xl sm:text-4xl font-bold flex items-center justify-center gap-2"
        >
          <span>Abdellatif Laghjaj </span>
          <svg
            class="w-8 h-8 sm:w-9 sm:h-9 mt-1"
            fill="currentColor"
            viewBox="0 0 20 20"
        ...
        <div class="mockup-code flex items-start flex-col my-3">
          <pre
            data-prefix="$"
          ><code>npm install <span class="text-success">@love.js</span> </code></pre>
          <pre
            data-prefix=">"
            class="text-warning"
          ><code>installing...<code>{{ installationPercentage }}</code></code></pre>
          <pre
            data-prefix=">"
            class="text-error"
            v-if="isInstalled"
          ><code>error: love not found</code></pre>
        </div>
        <label
          class="btn btn-primary modal-button w-full sm:w-auto"
          for="cv-modal"
        >
          <svg
            class="w-6 h-6"
